...
describe("reducer", () => {
  ...
  test("should remove the oldest query in the queue", () => {
    ...
    expect(
      reducer(
        fullQueueMockState,
        queryIsUpdated({ type: "name", value: "ing" })
      )
  ...
  test("should update both name and Tag properties and display student ids", () => {
    const newNameQueryState = reducer(
      mockState,
    ...
    expect(newNameQueryState).toStrictEqual(expectedNameQueryState);
    const searchState = reducer(
      newNameQueryState,
      queryIsUpdated({ type: "tag", value: "t1" })
    );
    const expectedState = produce(newNameQueryState, (draft) => {
      draft.tagQuery = "t1";
      draft.tagQueryCache.T1 = {
        ids: ["1", "3"],
        refCount: 1,
  ...
  test("should remove the oldest query in the queue", () => {
    ...
    expect(
      reducer(fullQueueMockState, queryIsUpdated({ type: "tag", value: "t1a" }))
    ).toStrictEqual(expectedState);
  });
